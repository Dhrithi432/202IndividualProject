...
public class JsonFileParser implements FileParser {
    ...
    public void fetchRecords(String inputFilename){
        ...
        //JSONObject jsonObject = (JSONObject) jsonParser.parse(new FileReader(inputFilename));
    ...
    public JSONObject processEachRecord(JSONObject record){
        ...
        try {
            card_number = (String) record.get("cardNumber").toString();
        }catch(Exception e){
        ...
        if(card==null){
            ...
            if(card_number.length() > 19) {
                error_message = "Invalid: more than 19 digits";
            }
